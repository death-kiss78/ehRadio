...
- Core macros:
  ...
  - Boot stage attribution: `BOOTTIMELOG(name)` closes each block of `setup()` with the cost of the stage that has just finished, e.g. `[BOOT]          01950ms: checkSpiffsandVer                    377ms` — the timestamp is the running total to the end of that stage and the message is that stage's share of it. It exists because the init paths announce completion with `BOOTLOGX` progress lines, which carry no stamp by design (a stamp would land mid-line before the dots), leaving the whole tail of `setup()` unmeasured: about 4.7 s of a 23.4 s boot on the 128x64 VS1053 build. Add a call after every block in `setup()` that can block; the first call measures from power-on. `SPIFFSTIMELOG` and `CONFIGTIMELOG` do the same for the SPIFFS and config paths, and all three live in `logging.cpp` behind declarations and macros in `logging.h` rather than as per-file twins. **Each keeps its own stamp**, so a marker measures against the previous marker of its own kind: the config and SPIFFS stages that run *inside* a `setup()` block would otherwise disturb the `setup()` deltas, and a single shared stamp would silently redefine every number in the log. `SPIFFSTIMELOGRESET()` and `CONFIGTIMELOGRESET()` restart the measurement at the entry to the functions whose first stage would otherwise be measured from the previous marker (`checkSpiffsandVer()`, `Config::init()`, `loadPreferences()`, `initPlaylistMode()`). The functions are declared whatever the build and compile to empty bodies without `BOOTLOG_TIME`, which is why the off build has no stage lines; the `verifySpiffs` summary line's `, NNNms` is gated the same way.
...
- Owns startup-time helpers that were previously mixed into `config.cpp`:
  - boot-time version marker and required SPIFFS/WebUI file verification (`checkSpiffsandVer()`) — the verification itself lives in `Utility::verifySpiffs()`, see the SPIFFS notes below
  - **SPIFFS lookup cost is proportional to partition size.** Measured on the sh1106_vs1053_3buttons build: a missing-name probe costs ~187 ms on the 3.38 MB partition in `builds/partitions/default_16MB.csv` and ~87 ms on the 1.5 MB one in `default_8MB.csv`, while `SPIFFS.begin()` mount moves 229 → 103 ms and the `netserver.begin()` file cache 797 → 382 ms — the same 2.25x factor, three independent measurements. SPIFFS proves a name exists, or does not, by walking the partition's lookup structures, so **the 8 MB partition table is the recommended layout**: the payload is well under 1 MB, and the 16 MB default pays ~2.25x on every file operation in the device for no benefit.
  - **Web-file verification is one listing pass, not N lookups.** `Utility::verifySpiffs()` in `utility.cpp` walks `/` once, matching every listed name against `Config::wwwFiles` as it goes, so it stores no names at all — only two bitmasks (plain seen, `.gz` seen) — and returns whether each required file is present in either form. Its single write is the duplicate rule: a plain file whose `.gz` twin exists is removed. It replaced two identical per-file probe loops (`requiredWebFilesExist()` in `startup.cpp` and `Config::_wwwFilesExist()`), which together cost **7.48 s of a 23.2 s boot** — 33 lookups each, run twice, because the second one re-answered a question `checkSpiffsandVer()` had already answered. `config.wwwFilesExist` is now computed once there and reused by `initPlaylistMode()`.
  - **`pruneSpiffs()` is a repair, not a verifier, and must never run on the boot path.** The old `cleanupSpiffs()` deletes everything outside the `wwwFiles`/`dataFiles` keep lists — which includes `/www/searchresults.json`, `/www/search.txt`, `/www/curated.json`, `/www/pl_import.json` and `/data/new_ver.txt` — so it stays on the missing-files and version-mismatch paths (and the `clearspiffs` command). It was renamed `pruneSpiffs()` so the pair reads as what they are: one verifies, one prunes.
...
- **The boot Wi-Fi path: what the scan costs, and what it cannot be reduced by.** Measured on the sh1106_vs1053_3buttons build once the SPIFFS work was fixed, `network.begin` was **12,278 ms of a 15.5 s boot** — `WiFi.scanNetworks()` 6609 ms plus the join 5523 ms — while the entire local bring-up (SPIFFS, player, battery, playlist, netserver cache) had fallen to 3.2 s. Of the changes made in `network.cpp` during this work, **two survive: `WIFI_SCAN_DWELL_MS` and `WIFI_CONNECT_POLL_MS`.** The asynchronous scan and the pinned static address are both gone from the tree, and the bullets below record what each of them taught.
  - **`WiFi.scanNetworks(async)` is NOT USABLE in this firmware — settled, and the attempt has been removed from the tree.** Three attempts, each eliminating one suspect, all failed identically: the async scan returns `WIFI_SCAN_FAILED` (-2) after ~2.5-6 s, and **every synchronous scan afterwards returns 0 networks in 2-12 ms** — the radio is left unable to scan at all, and `scanDelete()` does not clear it — so the fallback cannot recover and the boot lands in the SoftAP. What was ruled out across the three: (1) a driver call during the scan (`WiFi.mode()`/`setSleep()` mid-scan — fixed by collecting before any other call, still failed), (2) the per-channel dwell (120 and 300 both fail), (3) **clearing the modem-sleep policy before the scan** (attempt 3 left it entirely alone and still failed); (4) concurrency with flash-heavy work was present in all three, so it is not isolated, but it must not be assumed to be the cause either. The overlap it would have bought (~860 ms with today's ordering, ~3.2 s if the scan were started before `config.init()`) is not worth a radio that stops scanning: **do not re-add an async scan.**
  - `WiFi.setSleep(false)` is cleared **only after a successful connect**, in `setWifiParams()`, and nothing clears it before a connect. It was moved before the join once, on the theory that the ESP32 default (`WIFI_PS_MIN_MODEM`) duty-cycles the radio through association and DHCP; **the measurement does not support that theory and the move is not adopted.** The tally, including the boots that came after the move was reverted: cleared-before gave 4891-5000 ms DHCP on 5 boots out of 5; cleared-after gave 2 fast (1000, 501 ms) and 4 slow (4900, 5176, 5269, 5270 ms) over 6 boots. So a fast join is possible but rare, it has only been observed with the old placement, and the placement is **not** the controlling factor — an earlier conclusion here rested on 3 boots and the next 3 contradicted it. The join path is otherwise identical between those builds, which is the proof.
  - **The local "early" stage is not NVS-heavy — it is mostly the deliberate safe-mode delay.** The `cfgStage()` markers in `config.cpp` show, on a safe-mode boot: `config: safe-mode serial delay` **1000 ms**, `config: bootInfo (the config dump)` **196 ms** (baud-bound: ~40 lines at 115200), `config: rtc + SPI.begin + play_mode` 11 ms, `config: _initHW()` **0 ms**, and everything before the delay — serial, deassert, rgbled, battery and `loadPreferences()` with its `prefs.begin`, sentinel read, `keyMap` loop and `deleteOldKeys()` — about **412 ms**. So the ~1586 ms this stage reported boot after boot was `if (!config.store.bootStableMarker) delay(1000)` plus the config dump, and there is nothing worth optimising in the NVS path. **Measurement consequence:** `bootStableMarker` is only set ~10 s after the startup services finish, so rebooting the device within ~20 s puts *every* test boot into safe mode with that 1 s delay — nearly every boot measured during this work carried it, and a settled boot's early stage is ~590 ms.
  - **What is left in the boot, and what is not fixable.** Composition of a settled boot: scan **~5700 ms** (floored — ~320 ms per channel is fixed overhead the dwell cannot touch), DHCP **500-5300 ms** (environmental), association ~370 ms, local ~2400 ms. The only remaining lever is **DHCP**, and only via a static address, which would make the join ~380 ms regardless of how the AP behaves; the narrow-first scan was rejected deliberately because agile multiband / auto-channel-selection makes a last-known channel actively misleading rather than merely stale. That lever was built, measured on hardware and then removed: the address cannot be verified as free on this stack, which is the subject of the record below.
  - **A pinned static address was built, worked, and was removed — recorded here so it is not re-attempted.** The feature (`SMART_STATIC_IP`, a `myoptions.h` switch, all of its code inside `#ifdef` in `network.cpp`) remembered the DHCP lease once a boot had proved itself, applied it before the join, and judged the join on the wire instead of waiting for `WL_CONNECTED`. It worked and it was fast: pinned boots reached Ready in **4.6-4.8 s** in the good shape (netif up 0.43-0.74 s, join 1.1-1.35 s) and 5.4-6.8 s in the slow one, against ~8.7 s on the plain DHCP path, and **21 of 21 measured boots ended pinned** with the DHCP fallback never reached. The scan path never converted a first attempt (0 of the last 4) and stayed at 12.1-12.6 s Ready, dominated by the ~5.7 s scan.
  - **Why it was removed: this firmware cannot prove an address is free, and a wrong pin is silent.** lwIP here has **no ACD** (`LWIP_ACD` / `LWIP_DHCP_DOES_ACD_CHECK` are absent from every target's `sdkconfig`, so duplicate-address detection cannot be switched on without rebuilding the prebuilt IDF libraries), so the feature hand-rolled a probe: ARP the address, and treat any answer but our own MAC as a conflict. That check cannot work, and the field test proved it — with a PC holding `192.168.23.24`, first as a manual address and then as a DHCP reservation, the radio logged `confirmed on probe round 1: nobody else holds it` and used the address anyway. The cause: **an ARP reply whose sender IP equals the netif's own address is treated by lwIP as us**, so no foreign entry is ever cached under our own address and `etharp_find_addr()` cannot see the competing host — the contested branch was unreachable. What the probe *does* prove is that the address is bound and the gateway answers, i.e. that the subnet and the L2 path are the ones expected; it does not prove uniqueness. **The only safe deployment is an address reserved for the radio on the router by MAC, or one outside the DHCP pool — and the firmware cannot verify that**, which is why the feature is gone rather than shipped with a caveat.
  - **The durable findings from that work, which outlive it.** (1) `WL_CONNECTED` is set in exactly one place, on `ARDUINO_EVENT_WIFI_STA_GOT_IP` (`WiFiGeneric.cpp:1095`), and ESP-IDF posts it **0.5 s or 4.8 s after association** for a statically configured netif — the same bimodality the DHCP path shows, which is why the boot's "~4 s lost first attempt" was never the wire. (2) **The STA netif is raised by the DHCP client running, not by configuring an address.** A `WiFi.config()` before the connection only reaches esp_netif's *stored* copy while the lwIP netif still reads `0.0.0.0`, and a netif that is **down** cannot transmit at all (`etharp_request()` returning `ERR_OK` means "accepted for transmission", not "sent"), so the ARP probe silently failed for six boots while the flag said otherwise. (3) Three dead ends from that family, not to be re-tried: re-applying the address after association, "kicking" the DHCP client to raise the netif (11 boots, 10 still down — the bind's STOP cancels the kick's START in the same esp_netif queue), and priming every connect attempt (first attempts 2 of 12 with it, 4 of 11 without, ~1 s per blind boot).
  - **Build status.** `sh1106_vs1053_3buttons` builds clean at **1,832,285** bytes flash and **72,228** bytes RAM with the static-address feature removed and `BOOTLOG_TIME` off (1,827,041 / 72,244 in that state before the two later additions: the `msg_scanning_wifi` key across the 36 display locales, worth ~5 KB, and the three timing-log call sites, worth 52 bytes). The removal took `network.cpp` back to its pre-feature shape — the two original connect paths in `wifiBegin()`, and no `connectToCandidate()`, `linkUsable()`, `smartStaticSaveLease()` or `joinDiag()` — keeping only `WIFI_SCAN_DWELL_MS` and `WIFI_CONNECT_POLL_MS`. A build that dies at the `.bin` step (e.g. the uploader holding `firmware.bin`) leaves a stale sconsign, so the next build can link stale objects and report a plausible-looking size; delete `.pio\build\<env>\src\*.o` and rebuild rather than trusting it.
  - `WIFI_SCAN_DWELL_MS` (options.h, default **120**) replaces the hardcoded dwell. The scan is dominated by it: 13 channels measured 6609-7010 ms at the Arduino/IDF default of 300, so 120 lands near 4700 ms (measured ~5700 ms for the whole scan). It is safe to lower because `wifiBegin()` scans a second time at the default 300 when the configured dwell finds nothing, making the old behaviour the last resort — and only a failure *there* reaches the SoftAP. Its one earlier failure was **not** the dwell's fault: it was tested together with the broken asynchronous path, which is exactly the mistake to avoid here.
  - `WIFI_CONNECT_POLL_MS` (options.h, default **100**) replaces the hardcoded 500 ms poll, which had been quantising every connect measurement to half a second. The per-candidate ceiling is **time-based** (`WIFI_ATTEMPTS * 500`, the same 8 s as before) rather than attempt-based, because 16 attempts at a 100 ms poll would silently have cut it to 1.6 s. This is timing-only: the two connect paths in `wifiBegin()` are otherwise as they were before the static-address work.
  - **Session outcome: what the boot work bought, and what it cost.** Same hardware and network, start to end: **scan path 20-25 s to 9-15 s**, **blind path 12-15 s to 3.5-8 s**. The wins were of very different sizes: the scan dwell (~1 s), the connect poll (up to 400 ms per poll), `Utility::verifySpiffs()` replacing two per-file probe loops (3992 to 179 ms), the stage markers that showed *where* the time was, and the boot-line message that shows the scan instead of a blank screen. Two attempts did not survive, and both are recorded above: the **async scan** (wedges the radio) and the **pinned static address** (cannot be verified free - a former DHCP lease is not a safe static address, and its failure mode is a silent address collision on the LAN, not a slow boot). **Partition size matters less than it looks:** the same build on the 16 MB scheme versus the 8 MB one costs about **500 ms** in the SPIFFS path (mount 228 vs 102 ms, `verifySpiffs` 386 vs 179 ms, `checkSpiffsandVer` 737 vs 354 ms). The 2 s gap once seen between the schemes was not the partition: it was the pre-`verifySpiffs` file probing, which is exactly what the stage markers exist to expose.
  - **The join budget, and the retry that was lost with the static-address feature.** WL_CONNECTED is GOT_IP, so the per-candidate ceiling (WIFI_ATTEMPTS * 500 = 8 s) has to cover the association AND the lease, and the late GOT_IP mode measures 4.8 s on its own - a slow association plus a late flag therefore overflows it. One observed boot: a single blind candidate burned the whole 8 s (81 dots at 100 ms), the rotation wrapped back to the one saved SSID, wifiBegin() returned false and network.begin() raised the SoftAP. Both loops now log the association line (Associated after Nms (RSSI x) - waiting for the address) so a log splits association from lease, and both re-associate the same candidate once - disconnect, settle, begin again - before calling it failed, which is the behaviour the pin retry loop provided by accident and its retries reached the flag where first attempts did not. The deferred boot-line message always fires well before an AP fallback (2 s against a ceiling of 8 s per candidate), so it cannot land on the AP screen.
