...
void VuWidget::_draw(){
  ...
  if (config.store.vupeak) {
    ...
    uint16_t pkL = (_peakL < peakThk) ? peakThk : _peakL;
    uint16_t pkR = (_peakR < peakThk) ? peakThk : _peakR;
    if (_rotate) {
      ...
      if (!*boomboxStyle_ptr) {
        ...
        fillLocal(pkL - peakThk, 0, peakThk, thk, _vupeakcolor);
        fillLocal(cw - pkR, 0, peakThk, thk, _vupeakcolor);
      }
    } else {
      fillLocal(0, pkL - peakThk, _bands.width, peakThk, _vupeakcolor);
      fillLocal(thk + _bands.space, pkR - peakThk, _bands.width, peakThk, _vupeakcolor);
    }
  }
...
void VuWidget::loop(){
  /* Redraw ceiling.  The display task runs every DSP_TASK_DELAY (10 ms) and the widget used
     to redraw on every one of those, which is both wasteful and the reason the fade looked
     instant on fast panels.  VU_REFRESH_MS is chosen to keep up with the audio core's 30-50
     levels per second - go much slower and the peak marker starts missing transients. */
  uint32_t now = millis();
  if (_lastMs && (now - _lastMs) < VU_REFRESH_MS) return;
