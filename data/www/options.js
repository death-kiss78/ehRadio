...
/* The VU style list is generated by the device (/visuals.json), because only it knows which styles
   this build can draw: a VS1053 has no PCM, so the sample-based ones are simply absent from the map. */
async function loadVisuals() {
  try {
    const r = await fetch('visuals.json');
    if (!r.ok) throw new Error(`HTTP ${r.status}`);
    visualsData = await r.json();
    populateNamedDropdown('vustyle', visualsData);
    if (pendingVustyle !== null) {
      const sel = getId('vustyle');
      if (sel) sel.value = pendingVustyle;
      pendingVustyle = null;
    }
